...
use std::{default::Default, time::UNIX_EPOCH};
...
pub struct Config {
	...
	/// If set to `Some`, we'll use this to fetch preimages for incoming payments that we don't
	/// yet have the preimage for.
	pub unknown_preimage_fetcher_or: Option<Arc<dyn UnknownPreimageFetcher>>,
}
...
impl Default for Config {
	fn default() -> Self {
		Self {
			...
			unknown_preimage_fetcher_or: None,
		}
	}
}

/// Interface for fetching preimages for incoming payments that we don't yet have the preimage for.
#[async_trait]
pub trait UnknownPreimageFetcher: std::fmt::Debug + Send + Sync {
	/// Fetches the preimage for a given payment hash. Called by a [`Node`] instance when a
	/// claimable payment is received that we don't yet have the preimage for. This happens for
	/// payments created by `receive_payment_with_hash()` and `receive_variable_amount_payment_with_hash()`.
	/// If the preimage is returned, the payment will be claimed. If an error is returned, the
	/// payment will be rejected.
	async fn get_preimage(&self, payment_hash: PaymentHash) -> Result<PaymentPreimage, Error>;
}
...
impl<K: KVStore + Sync + Send + 'static> Node<K> {
	...
	pub fn start(&self) -> Result<(), Error> {
		...
		let event_handler = Arc::new(EventHandler::new(
			...
			match &self.unknown_preimage_fetcher_or {
				Some(ref fetcher) => Some(Arc::clone(fetcher)),
				None => None,
			},
		));
	...
	pub fn receive_payment(
		...
		self.receive_payment_inner(Some(amount_msat), description, expiry_secs, None)
	}
	...
	pub fn receive_variable_amount_payment(
		...
		self.receive_payment_inner(None, description, expiry_secs, None)
	}

	/// Returns a payable invoice that can be used to request and receive a payment of the amount
	/// given, using the given payment hash. This is useful when the pre-image was generated by
	/// another party and is not yet known, and allows for the pre-image to be fetched out of band
	/// when the invoice is paid. [`Node`] must have been configured with an
	/// [`UnknownPreimageFetcher`] for this to work. If no such fetcher is configured, payments
	/// received from invoices generated with this method will be rejected.
	pub fn receive_payment_with_hash(
		&self, amount_msat: u64, description: &str, expiry_secs: u32, payment_hash: PaymentHash,
	) -> Result<Bolt11Invoice, Error> {
		self.receive_payment_inner(Some(amount_msat), description, expiry_secs, Some(payment_hash))
	}

	/// Returns a payable invoice that can be used to request and receive a payment for which the
	/// amount is to be determined by the user, also known as a "zero-amount" invoice, using the
	/// given payment hash. This is useful when the pre-image was generated by another party and is
	/// not yet known, and allows for the pre-image to be fetched out of band when the invoice is
	/// paid. [`Node`] must have been configured with an [`UnknownPreimageFetcher`] for this to
	/// work. If no such fetcher is configured, payments received from invoices generated with this
	/// method will be rejected.
	pub fn receive_variable_amount_payment_with_hash(
		&self, description: &str, expiry_secs: u32, payment_hash: PaymentHash,
	) -> Result<Bolt11Invoice, Error> {
		self.receive_payment_inner(None, description, expiry_secs, Some(payment_hash))
	}

	/// Returns a payable invoice that can be used to request and receive a payment.
	///
	/// * `amount_msat` - The amount in millisatoshi to be paid. If `None`, the
	///   amount is to be determined by the user, also known as a "zero-amount"
	///   invoice.
	/// * `description` - A description of the payment.
	/// * `expiry_secs` - The number of seconds after which the invoice expires.
	/// * `payment_hash` - The payment hash to use for the invoice. If `None`, a
	///   random pre-image and corresponding payment hash are generated and the
	///   pre-image is saved in the payment store. If `Some`, the given payment
	///   hash is used and no pre-image is saved in the payment store. This is
	///   useful when the pre-image was generated by another party and is not
	///   yet known, and allows for the pre-image to be fetched out of band when
	///   the invoice is paid.
	fn receive_payment_inner(
		&self, amount_msat: Option<u64>, description: &str, expiry_secs: u32,
		payment_hash_or: Option<PaymentHash>,
	) -> Result<Bolt11Invoice, Error> {
		let currency = Currency::from(bitcoin::Network::from(self.config.network));
		let keys_manager = Arc::clone(&self.keys_manager);
		let invoice_or = match payment_hash_or {
			Some(payment_hash) => {
